...
namespace DBProject.DAL
{
	...
	public class myDAL
    {
        ...
        private static readonly string connString = System.Configuration.ConfigurationManager.ConnectionStrings["sqlCon4"].ConnectionString;
        ...
        public int patientInfoDisplayer(int pid, ref string name, ref string phone, ref string address, ref string birthDate, ref int age, ref string gender)
		{
			...
			try
			{
                ...
                /* GETTING OUTPUTS*/
                if (cmd1.Parameters["@name"].Value != System.DBNull.Value)
                    name = (string)cmd1.Parameters["@name"].Value;

                if (cmd1.Parameters["@phone"].Value != System.DBNull.Value)
                    phone = (string)cmd1.Parameters["@phone"].Value;

                if (cmd1.Parameters["@address"].Value != System.DBNull.Value)
                    address = (string)cmd1.Parameters["@address"].Value;

                if (cmd1.Parameters["@birthDate"].Value != System.DBNull.Value)
                    birthDate = (string)cmd1.Parameters["@birthDate"].Value;

                if (cmd1.Parameters["@age"].Value != System.DBNull.Value)
                    age = (int)cmd1.Parameters["@age"].Value;

                if (cmd1.Parameters["@gender"].Value != System.DBNull.Value)
                    gender = (string)cmd1.Parameters["@gender"].Value;
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Doctor] ORDER BY [Name]", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Doctor] WHERE [DoctorId] = @id", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Appointment] WHERE [AppointmentId] = @id", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
            da.SelectCommand.Parameters.AddWithValue("id", id);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Appointment] WHERE [DoctorId] = @doctor AND NOT (([AppointmentEnd] <= @start) OR ([AppointmentStart] >= @end))", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Appointment] WHERE NOT (([AppointmentEnd] <= @start) OR ([AppointmentStart] >= @end))", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
            da.SelectCommand.Parameters.AddWithValue("start", start);
            da.SelectCommand.Parameters.AddWithValue("end", end);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        {
            //SqlDataAdapter da = new SqlDataAdapter("SELECT * FROM [Appointment] WHERE ([AppointmentStatus] = 'free' OR ([AppointmentStatus] <> 'free' AND [AppointmentPatientSession] = @session)) AND NOT (([AppointmentEnd] <= @start) OR ([AppointmentStart] >= @end))", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
        ...
        {
            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString))
            {
                con.Open();
        ...
        {
            SqlDataAdapter da = new SqlDataAdapter("SELECT top 1 * FROM [Doctor] ORDER BY [Name]", ConfigurationManager.ConnectionStrings["sqlCon1"].ConnectionString);
